...
# (version Python 3.12) otherwise the program will look in your
# root directory for the text files.
...
"""Each assigned task's details will be stored in a dictionary
in task.txt."""
...
a different username.\n")
                try:
                    register_or_main_menu = int(input("Enter any number to \
register a new user or -1 to return to Main Menu:  "))
                    if register_or_main_menu == -1:
                        break 
                    else:
                        continue
...
register a new user or -1 to return to Main Menu:  "))
                        ...
                        print("\nThis is an invalid entry.  Enter any number \
to register a new user or -1 for the Main Menu.")  
                        continue
...
def view_all():
    '''Ths function reads all tasks from task.txt file and prints to
    the screen.
    '''
    print("\n---------------------------------------------------------------")
...
complete (m) or edit the task (e) or quit to return to the Main Menu (q)? \
(Enter m or e or q):  ").lower()
            
                    if mark_or_edit == "q":
                        break
                    elif mark_or_edit != "m" or mark_or_edit != "e":
                        break
                    else:
                        continue
...
task username (u) or due date (d) or quit (q) to return to the Main Menu? \
(Enter u or d or q):  ").lower() 
                    
                    if edit_task == "q":
                        break
                    elif edit_task != "u" or edit_task != "d":
                        break
                    else:
                        continue
...
\nUpdated due date is:\t{edit_due_date}.") 
                ...
                # If user chooses to quit editing the task and return to Main Menu.
                elif edit_task == "q":
                    break
                ...
                """ 
            # If user chooses to quit marking or editing and return to Main Menu.
            elif mark_or_edit == "q": 
                break
                              
            else:
                print("\nInvalid entry. Unable to edit this task.  The task has already been \
...
def generate_user_report():
    ...
    This generate_user_report function is called to from the
    generate_reports function.
    """
     
    current_date_time = datetime.today()
    
    # The user_overview string will be written to user_overview.txt.
    # Please note that each line of the string has not been continued
    # on the next line despite extending over 79 characters, for easy
    # readability purposes.
    user_overview = "\n---------------------------- User Overview Report --------------------------------\n"
    user_overview += f"\nDate and time of User Overview Report: {current_date_time}\n"
    ...
    # Loop through users in dictionary to separate tasks by their assigned user.
    for username in username_password.keys(): 
        ...
        """Looping through tasks and counting total tasks,
        completed tasks, incomplete tasks and overdue tasks
        for each username.
        """
        for t in task_list:
            ...
            if t["username"] == username: 
                ...
                if t["completed"] is True:
                    ...
                    if current_date_time > t["due_date"]:
    ...
    # Writing the string user_overview to text file to create the User Overview Report.       
...
def generate_reports():
    ...
    task_overview.txt. This function will generate the 
    Task Overview Report in the text file 'task_overview.txt'.
    
    The generate_user_report function is called to when the user
    selects gr (generate reports).
    This will generate the User Overview Report in the text file 
    'user_overview.txt' 
...
Main Menu.\n")
        ...
        for t in task_list:
            ...
            if t["completed"] is True:
                ...
                # Counter increases by 1 for each incomplete and overdue task.    
                if current_date_time > t["due_date"]: 
                    overdue_tasks += 1   
                    
        """String created using counter variables from looping through
        task_list.
        The string task_overview will be written to task_overview.txt.
        ...
        task_overview += f"\nDate and time of Task Overview Report: {current_date_time}\n"
        task_overview += f"\nTotal number of tasks generated and tracked by task_manager.py:\t\t{str(len(task_list))}"
...
def display_statistics():  
    '''This function will display on screen, general statistics
    (User Overview), Task Oveview Report and User Overview Report
    for admin only.
    '''

    if curr_user == "admin":
    
        """Calling to generate_reports function.  If admin is the
        current user, Task Overview Report and User Overview Report
        will be generated if they have not already been generated.
        """
        generate_reports() 
    

        '''If the user is an admin they can view statistics about
        number of users and tasks (User Overview).'''
        ...
        """Printing Task Overview Report to the screen.
        
        Due to tabs used in the Task Overview Report to align numbers
        and percentages to the right, the lines being read from the
        report will need to be formatted by adjusting the tabs to
        ensure correct alignment of numbers and percentages on screen.
        """
        with open('task_overview.txt', 'r', encoding = 'utf-8') as tasks_file:  # Opening the task_overview.txt file to read.
            for line in tasks_file:
                if "generated" in line:
                    print(line.strip().replace("\t", "").replace(":",":\t\t"))
                elif "overdue tasks" in line:
                    print(line.strip().replace("\t", "").replace(":",":\t\t\t\t"))
                elif "Total number of" in line:
                    print(line.strip().replace("\t", "").replace(":",":\t\t\t\t\t"))
                elif "Percentage of incomplete tasks" in line:
                    print(line.strip().replace("\t", "").replace(":",":\t\t\t\t\t\t"))
                elif "Percentage of tasks that are overdue" in line:
                    print(line.strip().replace("\t", "").replace(":",":\t\t\t\t\t"))
                else:
                    print(line.strip())
        ...
        """Printing User Overview Report to the screen.
        
        Due to tabs used in the User Overview Report to align numbers
        and percentages to the right, the lines being read from the
        report will need to be formatted by adjusting the tabs to
        ensure correct alignment of numbers and percentages on screen.
        """
        with open('user_overview.txt', 'r', encoding = 'utf-8') as users_file: # Opening the task_overview.txt file to read.
